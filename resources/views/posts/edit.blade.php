<x-layout>
    <div class="max-w-3xl mx-auto my-8">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edytuj post</h1>

        <form method="POST" action="{{ route('posts.update', $post->slug) }}" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach($categories as $category)
                        <label class="inline-flex items-center bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition {{ in_array($category->id, old('categories', $post->categories->pluck('id')->toArray())) ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-300 dark:border-indigo-700' : '' }}">
            ...
            <div>
                ...
                <textarea name="lead" rows="3" 
                    class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('lead', $post->lead) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Treść</label>
                <textarea name="content" rows="8" 
                    class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('content', $post->content) }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_published" class="ml-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">Opublikuj</label>
                </div>
            </div>
                ...
                @if($post->photo)
                    <div id="current-photo" class="mb-4">
                        ...
                        <label class="inline-flex items-center mt-2 cursor-pointer">
                            <input type="checkbox" name="remove_photo" value="1" class="rounded border-gray-300 dark:border-gray-500 text-red-500 dark:text-red-400 focus:ring-red-500 mr-2">
                            <span class="text-sm text-red-600 dark:text-red-400">Usuń zdjęcie</span>
                        </label>
                    </div>
                @endif

                <div id="drop-zone" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg hover:border-indigo-400 transition cursor-pointer bg-gray-50 dark:bg-gray-700 {{ $post->photo ? 'hidden' : '' }}">
            ...
            <div class="flex items-center gap-4 pt-4">
                ...
                <a href="{{ route('posts.show', $post->slug) }}" class="px-6 py-3 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    Anuluj
                </a>
            </div>
        </form>
