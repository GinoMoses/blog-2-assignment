<x-layout>
    <div class="max-w-3xl mx-auto my-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Nowy post</h1>

        <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach($categories as $category)
                        <label class="inline-flex items-center bg-gray-50 border border-gray-200 rounded-full px-4 py-2 cursor-pointer hover:bg-gray-100 transition">
            ...
            <div>
                ...
                <textarea name="lead" rows="3" 
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('lead') }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Treść</label>
                <textarea name="content" rows="8" 
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('content') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-4 pt-4">
                ...
                <a href="{{ route('posts.index') }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                    Anuluj
                </a>
            </div>
        </form>
